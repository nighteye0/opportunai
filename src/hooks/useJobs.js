...
async function fetchRemotive() {
  try {
    const url = IS_PROD ? '/api/jobs' : `${PROXY}https://remotive.com/api/remote-jobs?limit=20`
    if (IS_PROD) return [] // handled by /api/jobs
...
export function useJobs({ query = '', source = 'all', type = 'all', tab = 'all', extraJobs = [] } = {}) {
  ...
  useEffect(() => {
    ...
    if (IS_PROD) {
      ...
      fetchAllFromAPI().then(apiJobs => {
        setJobs([...COMMUNITY_JOBS, ...extraJobs, ...apiJobs])
        setLoading(false)
      })
    } else {
      ...
      Promise.allSettled([
        ...
        const failed = results
          .map((r, i) => r.status === 'rejected'
            ? ['Remotive', 'Arbeitnow', 'RemoteOK', 'Himalayas', 'Jobicy'][i]
            : null)
          .filter(Boolean)
        setErrors(failed)
        setJobs(allJobs)
