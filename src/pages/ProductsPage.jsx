...
function getUrl(p) {
  return p.url || p.website || p.website_url || p.link || p.href || p.external_url || p.affiliate_url || p.product_url || p.buy_url || p.gumroad_url || null
}
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    if (search.trim()) { const q=search.toLowerCase(); arr=arr.filter(p=>(p.name||'').toLowerCase().includes(q)||(p.description||'').toLowerCase().includes(q)) }
    setFiltered(arr)
  }, [cat,search,products])
  const featured = products.filter(p=>p.featured).slice(0,4)
  const pill = a => ({ padding:'7px 16px', borderRadius:100, fontSize:13, fontWeight:500, border:`1px solid ${a?'rgba(245,158,11,0.3)':'rgba(255,255,255,0.07)'}`, background:a?'rgba(245,158,11,0.08)':'transparent', color:a?'#f59e0b':'#555', cursor:'pointer', fontFamily:'inherit', transition:'all 0.15s', whiteSpace:'nowrap' })
  const Card = ({p, feat}) => (
    <a href={getUrl(p)||'#'} target="_blank" rel="noopener noreferrer"
      style={{background:'#0a0a0a',border:`1px solid ${feat?'rgba(245,158,11,0.12)':'rgba(255,255,255,0.07)'}`,borderRadius:12,padding:18,display:'block',textDecoration:'none',color:'inherit',transition:'all 0.2s',position:'relative'}}
      onMouseEnter={e=>{e.currentTarget.style.borderColor=feat?'rgba(245,158,11,0.25)':'rgba(255,255,255,0.14)';e.currentTarget.style.transform='translateY(-2px)';e.currentTarget.style.boxShadow='0 12px 32px rgba(0,0,0,0.4)'}}
      onMouseLeave={e=>{e.currentTarget.style.borderColor=feat?'rgba(245,158,11,0.12)':'rgba(255,255,255,0.07)';e.currentTarget.style.transform='none';e.currentTarget.style.boxShadow='none'}}
    >
      {feat&&<span style={{position:'absolute',top:12,right:12,padding:'2px 8px',background:'rgba(245,158,11,0.1)',border:'1px solid rgba(245,158,11,0.2)',borderRadius:5,fontSize:11,fontWeight:700,color:'#f59e0b'}}>Featured</span>}
      ...
      <div style={{fontSize:13,color:'#555',lineHeight:1.6,marginBottom:14,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{p.description}</div>
        ...
        <div style={{maxWidth:1120,margin:'0 auto'}}>
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',flexWrap:'wrap',gap:16,marginBottom:24}}>
            ...
            <div style={{display:'flex',gap:8}}>
              <input placeholder="Search products..." value={search} onChange={e=>setSearch(e.target.value)} style={{padding:'10px 16px',background:'#0f0f0f',border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,fontSize:14,color:'#e8e8e8',fontFamily:'inherit',outline:'none',width:220}}/>
        ...
        {cat==='All'&&!search&&featured.length>0&&(
          <div style={{marginBottom:48}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:10,marginBottom:32}}>
              {featured.map((p,i)=><Card key={i} p={p} feat/>)}
            </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:10}}>
          {loading ? Array.from({length:8}).map((_,i)=>(
            <div key={i} style={{background:'#0a0a0a',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:18,opacity:0.2}}>
              ...
              <div style={{height:11,background:'#1a1a1a',borderRadius:4,width:'90%'}}/>
            </div>
