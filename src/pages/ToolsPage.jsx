...
function getUrl(tool) {
  ...
  return tool.url || tool.website || tool.website_url || tool.link ||
    tool.href || tool.external_url || tool.affiliate_url ||
    tool.product_url || tool.visit_url || null
}
...
export default function ToolsPage() {
  ...
  useEffect(() => {
    fetch('/api/tools')
      ...
      .then(d => {
        ...
        console.log('Tools sample:', arr[0]) // debug: shows field names
        setTools(arr)
        setFiltered(arr)
        setLoading(false)
      })
  ...
  useEffect(() => {
    ...
    if (search.trim()) {
      ...
      arr = arr.filter(t =>
        ...
        (t.description || t.tagline || t.short_description || '').toLowerCase().includes(q)
      )
    }
    setFiltered(arr)
  }, [cat, search, tools])
  ...
  const handleToolClick = (tool) => {
    ...
    if (url) {
      ...
      console.warn('No URL found for tool:', tool)
    }
  }
      ...
      <div style={{ maxWidth: 1120, margin: '0 auto', padding: '28px 24px 80px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 10 }}>
          {loading
            ...
            : filtered.map((t, i) => {
              const url = getUrl(t)
              const desc = t.description || t.tagline || t.short_description || ''
              return (
                <div
                  key={i}
                ...
                >
                  ...
                  {url && (
                    <div style={{ position: 'absolute', top: 14, right: 14, fontSize: 11, color: '#333' }}>↗</div>
                  )}
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    {t.category && (
                      <span style={{ padding: '2px 8px', background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.12)', borderRadius: 5, fontSize: 11, fontWeight: 600, color: '#555' }}>
                        {t.category}
                      </span>
                    )}
                    {url && (
