...
function getUrl(t) {
  return t.url || t.website || t.website_url || t.link || t.href || t.external_url || null
}
export default function ToolsPage() {
  ...
  useEffect(() => {
    ...
    if (search.trim()) { const q=search.toLowerCase(); arr=arr.filter(t=>(t.name||'').toLowerCase().includes(q)||(t.description||t.tagline||'').toLowerCase().includes(q)) }
    setFiltered(arr)
  }, [cat,search,tools])
  const pill = a => ({ padding:'7px 16px', borderRadius:100, fontSize:13, fontWeight:500, border:`1px solid ${a?'rgba(99,102,241,0.3)':'rgba(255,255,255,0.07)'}`, background:a?'rgba(99,102,241,0.08)':'transparent', color:a?'#818cf8':'#555', cursor:'pointer', fontFamily:'inherit', transition:'all 0.15s', whiteSpace:'nowrap' })
  return (
      ...
      <div style={{padding:'48px 24px 36px',borderBottom:'1px solid rgba(255,255,255,0.07)',background:'radial-gradient(ellipse 70% 80% at 50% 0%,rgba(99,102,241,0.04),transparent)'}}>
        <div style={{maxWidth:1120,margin:'0 auto'}}>
          ...
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
            ...
            <input style={{padding:'10px 16px',background:'#0f0f0f',border:'1px solid rgba(255,255,255,0.07)',borderRadius:9,fontSize:14,color:'#e8e8e8',fontFamily:'inherit',outline:'none',width:240,maxWidth:'100%'}} placeholder="Search tools..." value={search} onChange={e=>setSearch(e.target.value)} />
          </div>
        </div>
      </div>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))',gap:10}}>
          {loading ? Array.from({length:9}).map((_,i)=>(
            <div key={i} style={{background:'#0a0a0a',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:18,opacity:0.2}}>
              ...
              <div style={{height:12,background:'#1a1a1a',borderRadius:4,width:'90%'}}/>
            </div>
          )) : filtered.map((t,i)=>(
            <a key={i} href={getUrl(t)||'#'} target="_blank" rel="noopener noreferrer"
              style={{background:'#0a0a0a',border:'1px solid rgba(255,255,255,0.07)',borderRadius:12,padding:18,display:'block',textDecoration:'none',color:'inherit',transition:'all 0.2s',position:'relative'}}
            ...
            >
              ...
              <div style={{width:40,height:40,borderRadius:9,background:'#111',border:'1px solid rgba(255,255,255,0.06)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,marginBottom:12}}>{t.logo||t.emoji||t.icon||'🛠️'}</div>
              <div style={{fontSize:15,fontWeight:600,color:'#e0e0e0',marginBottom:4}}>{t.name}</div>
              <div style={{fontSize:13,color:'#444',lineHeight:1.6,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden',marginBottom:10}}>{t.description||t.tagline||t.short_description}</div>
